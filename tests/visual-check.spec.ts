...
test("ビジュアル確認: プレビューとダウンロード画像の比較", async ({ page }) => {
  ...
  // プレビュー画像の読み込みを待つ
  await page.waitForSelector('img[alt*="Background"]', { state: "visible", timeout: 10000 });
  ...
  console.log("[ビジュアル確認] ✓ プレビューのスクリーンショット保存: test-results/preview-screenshot.png");

  // ダウンロードボタンをクリック
  const downloadPromise = page.waitForEvent("download", { timeout: 15000 });
  await page.click('button:has-text("ダウンロード")');
  const download = await downloadPromise;
  console.log(`[ビジュアル確認] ✓ ダウンロード開始: ${download.suggestedFilename()}`);
