  ...
  try {
    ...
    const domInfo = await page.evaluate(() => {
      return {
        ...
        hasDebugPanel: !!document.querySelector(
          '[style*="position: fixed"][style*="bottom: 0"]',
        ),
        imageCount: document.querySelectorAll("img").length,
        errorDivs: Array.from(
          document.querySelectorAll('[style*="rgba(255,0,0"]'),
        ).map((el) => el.textContent),
        bodyClasses: document.body.className,
        hasBackgroundPreview: !!document.querySelector('[id^="preview-"]'),
        hasDownloadElements:
          document.querySelectorAll('[id^="download-"]').length,
      };
    });
    ...
    console.log(
      `  - Background preview: ${domInfo.hasBackgroundPreview ? "✅" : "❌"}`,
    );
    ...
    console.log(
      `  - Network errors: ${errors.network.filter((e) => !e.url?.includes("favicon")).length}`,
    );
