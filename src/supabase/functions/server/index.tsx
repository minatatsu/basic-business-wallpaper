...
app.get("/make-server-b35d308f/figma/images", async (c) => {
  try {
    ...
    console.log(
      `Requesting Figma images for ${nodeIds.split(",").length} nodes (format: ${format || "url"})...`,
    );
        ...
        for (const [nodeId, imageUrl] of Object.entries(data.images)) {
          if (typeof imageUrl === "string") {
            try {
              ...
              if (!imgResponse.ok) {
                console.error(
                  `Failed to fetch image for ${nodeId}:`,
                  imgResponse.status,
                );
                continue;
              }
              ...
              console.log(
                `✓ Converted ${nodeId} to base64 (${base64.length} chars)`,
              );
            } catch (err) {
