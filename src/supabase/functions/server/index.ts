...
app.get("/make-server-b35d308f/figma/images", async (c) => {
  try {
    ...
    console.log(
      `Requesting Figma images for ${nodeIds.split(",").length} nodes...`,
    );
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return c.json(
          { error: "Figma API error", details: errorText },
          response.status,
        );
      }
      ...
      console.log(
        `✓ Successfully fetched ${Object.keys(data.images || {}).length} image URLs`,
      );

      // Optionally convert images to base64 if requested
        ...
        for (const [nodeId, imageUrl] of Object.entries(data.images)) {
          try {
            ...
            if (imgResponse.ok) {
              ...
              base64Images[nodeId] = `data:${contentType};base64,${base64}`;
            } else {
              base64Images[nodeId] = imageUrl as string;
            }
          } catch (err) {
            console.error(`Failed to convert image ${nodeId} to base64:`, err);
            base64Images[nodeId] = imageUrl as string;
          }
        }
        return c.json({ ...data, images: base64Images });
      }
...
app.get("/make-server-b35d308f/figma/nodes", async (c) => {
  try {
    ...
    console.log(
      `Requesting Figma nodes for ${nodeIds.split(",").length} nodes...`,
    );
    ...
    try {
      ...
      const data = await response.json();
      console.log(
        `✓ Successfully fetched ${Object.keys(data.nodes || {}).length} node structures`,
      );
      return c.json(data);
    } catch (fetchError) {
      clearTimeout(timeoutId);
...
app.get("/make-server-b35d308f/image-proxy", async (c) => {
  try {
    ...
    console.log(
      `✓ Successfully proxied image (${arrayBuffer.byteLength} bytes)`,
    );
