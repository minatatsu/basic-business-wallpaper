...
export async function generatePNG(
  ...
  console.log(
    `[ImageGenerator] Generating PNG for template: ${templateId} (html2canvas)`,
  );
  ...
  if (!templateData) {
    console.error(
      "[ImageGenerator] Available templates:",
      Object.keys(globalTemplateData),
    );
    throw new Error(
      `Template data not found: ${templateId}. Did you call setTemplateData()?`,
    );
  }
  ...
  if (!downloadElement) {
    throw new Error(
      `Download canvas element not found: download-${templateId}. Make sure the template is selected.`,
    );
  }
  ...
  try {
    ...
    console.log(
      `[ImageGenerator] ✓ Canvas created: ${canvas.width}x${canvas.height}`,
    );
...
export async function generateZip(
  ...
  console.log(
    `[ImageGenerator] Starting ZIP generation for ${selectedTemplates.length} templates`,
  );
  ...
  if (errors.length > 0) {
    console.error(
      `[ImageGenerator] ${errors.length} templates failed:`,
      errors,
    );
    throw new Error(
      `Failed to generate ${errors.length} images. Check console for details.`,
    );
  }
