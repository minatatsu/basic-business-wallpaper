  ...
  return fetchWithRetry(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to fetch Figma images: ${error.error || response.statusText}`);
      }

      const data = await response.json();
      console.log('[FigmaAPI] ✓ Successfully fetched Figma images:', Object.keys(data.images || {}).length, 'images');
      ...
      if (firstImageUrl && firstImageUrl.startsWith('data:')) {
        ...
        console.log('[FigmaAPI] ⚠ Edge Function returned raw URLs, converting to base64 on client...');
        ...
        for (const [nodeId, imageUrl] of Object.entries(images)) {
          ...
          try {
            ...
            if (!imgResponse.ok) {
              console.error(`[FigmaAPI] Failed to fetch image for ${nodeId}: ${imgResponse.status}`);
              continue;
            }
            ...
            console.error(`[FigmaAPI] Error converting ${nodeId} to base64:`, err);
          }
        }

        console.log(`[FigmaAPI] ✓ Converted ${Object.keys(base64Images).length} images to base64`);
        return base64Images;
      }
    } catch (error) {
      clearTimeout(timeoutId);
      if (error instanceof Error && error.name === 'AbortError') {
        ...
        throw new Error('Request timed out - Figma API is taking too long to respond');
      }
  ...
  return fetchWithRetry(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.json();
        console.error('[FigmaAPI] Error response:', error);
        throw new Error(`Failed to fetch Figma nodes: ${error.error || response.statusText}`);
      }

      const data = await response.json();
      console.log('[FigmaAPI] ✓ Successfully fetched Figma nodes:', Object.keys(data.nodes || {}).length, 'nodes');
      return data.nodes || {};
    } catch (error) {
      clearTimeout(timeoutId);
      if (error instanceof Error && error.name === 'AbortError') {
...
export function findAutoLayoutFrames(node: any, textLayerIds: Set<string>): Record<string, any> {
