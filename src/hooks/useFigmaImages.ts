...
import { fetchFigmaImageUrls, fetchFigmaNodes, findTextLayers, findAutoLayoutFrames } from '../utils/figmaApi';
import { TEMPLATES, TemplateData, TextLayerInfo, FrameLayoutInfo } from '../types';
    ...
    async function loadTemplates() {
      try {
        ...
        const timeoutPromise = new Promise((_, reject) => 
          setTimeout(() => reject(new Error('Request timed out - please check your connection and try again')), 90000)
        );
        
        const [urls, nodes] = await Promise.race([
          Promise.all([
            fetchFigmaImageUrls(nodeIds),
            fetchFigmaNodes(nodeIds)
          ]),
          timeoutPromise
        ...
        TEMPLATES.forEach(template => {
          ...
          console.log(`Template ${template.id}:`, { nodeIdWithColon, nodeIdWithDash, imageUrl: imageUrl?.substring(0, 50) + '...', availableKeys: Object.keys(urls) });
          if (!imageUrl) {
            console.warn(`No image URL found for template ${template.id} (node: ${template.nodeId})`);
            return;
          }

          // Edge Function already converts to base64, no proxy needed
          if (imageUrl.startsWith('data:')) {
            console.log(`✓ Using base64 for ${template.id}:`, imageUrl.substring(0, 50) + '...');
          } else {
            console.warn(`⚠ Unexpected non-base64 URL for ${template.id}:`, imageUrl.substring(0, 100));
          }
          ...
          const textLayerIds = new Set(Object.values(textLayersRaw).map((l: any) => l.id));
          const framesRaw = findAutoLayoutFrames(nodeData.document, textLayerIds);
          ...
          Object.entries(textLayersRaw).forEach(([fieldName, layer]: [string, any]) => {
            ...
            if (style.lineHeightPx) {
              ...
              lineHeight = { unit: 'PERCENT', value: style.lineHeightPercentFontSize };
          ...
          Object.entries(framesRaw).forEach(([frameId, frame]: [string, any]) => {
